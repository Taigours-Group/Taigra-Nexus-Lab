...
export const PageHero = ({ title, highlight, subtitle, badge, children }) => {
  return (
    <section className="relative overflow-hidden border-b border-white/[0.06] hero-mesh">
      ...
      <div className="container-page py-12 sm:py-16 md:py-24 relative z-10 text-center">
        {badge && (
          <motion.span
            ...
            className="inline-block mb-6 px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider text-blue-400 bg-blue-500/10 border border-blue-500/20"
          >
            {badge}
          </motion.span>
        )}
        <motion.h1
          ...
          className="text-2xl sm:text-4xl md:text-5xl font-bold tracking-tight text-white mb-4 sm:mb-5 leading-[1.15] px-1"
        >
          {title}{' '}
...
export const SectionHeading = ({ title, subtitle, action }) => (
  <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-10 md:mb-14">
    <div>
      ...
      {subtitle && <p className="text-slate-400 text-sm md:text-base max-w-xl">{subtitle}</p>}
    </div>
    {action}
  </div>
);
...
export const BreadcrumbBar = ({ items }) => (
  <nav aria-label="Breadcrumb" className="container-page py-2.5 sm:py-3 border-b border-white/[0.06] bg-[#0a0e18]/80">
